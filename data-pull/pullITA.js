...
(async () => {
	...
	var response = await axios.get('https://raw.githubusercontent.com/pcm-dpc/COVID-19/master/dati-regioni/dpc-covid19-ita-regioni-latest.csv');
	...
	const provincePromise = async () => {
		...
		response = await axios.get('https://raw.githubusercontent.com/pcm-dpc/COVID-19/master/dati-province/dpc-covid19-ita-province-latest.csv');
	...
	const vaccinePromise = async () => {
		...
		response = await axios.get('https://raw.githubusercontent.com/italia/covid19-opendata-vaccini/master/dati/somministrazioni-vaccini-latest.csv');
	...
	fs.writeFileSync('assets/data/ITA.json', JSON.stringify(result));
	tools.msg.log('Done!');
})();
