		...
		layer.on({
			...
			click: () => {
				var fxProvince = () => {
					...
					$('.lastUpdated').html(casesITA.proLastUpdate);
				...
				var fxRegione = () => {
					...
					$('.lastUpdated').html(casesITA.regLastUpdate);
					bgColor = mapStyle.incidence(toPrint.cases7 / toPrint.EWZ * 100000);
				};
		...
		layer.on({
			...
			click: () => {
				var fxProvince = () => {
					...
					$('.lastUpdated').html(vacITA.lastUpdate);
...
const pullITA = async () => {
	...
	casesITA.proLastUpdate = result.lastUpdate.pro;
	casesITA.regLastUpdate = result.lastUpdate.reg;
	vacITA.lastUpdate = result.lastUpdate.vac;
};
